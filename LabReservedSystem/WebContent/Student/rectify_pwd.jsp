...
function rectifySubmit(){
	...
	var loader=new net.AjaxRequest("../StudentServlet?action=change_password&nocache="+new Date().getTime(),deal_change_password,onerror,"POST",encodeURI(param));
}
function onerror(){
	alert("您的操作有误！");
}
function deal_change_password(){
	...
	alert(h);
	if(h=="修改成功！"){
		window.top.location.href="studentLogin.jsp";
	}else{
		document.getElementById("old_password").value="";
		document.getElementById("new_password").value="";//清空密码文本框
		document.getElementById("confirm_password").value="";
